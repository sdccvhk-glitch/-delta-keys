...
## UI/admin fix
- All normal text now uses white/light text for the dark theme.
- Admin login is available at `/admin-login.html` and `/admin` on static hosting.
- Admin dashboard fallback is `/admin-dashboard.html`.
- The Express API is still required for real admin authentication, products, orders and key inventory; Netlify static hosting alone cannot run `server.js`.
