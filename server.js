...
async function telegramApiMultipart(method, fields, fileField, fileName, fileBuffer, mimeType = "image/png") {
  if (!TELEGRAM_BOT_TOKEN) throw new Error("TELEGRAM_BOT_TOKEN is not configured.");

  const form = new FormData();
  for (const [key, value] of Object.entries(fields || {})) form.append(key, key === "reply_markup" && value && typeof value === "object" ? JSON.stringify(value) : String(value));
...
async function sendTelegramProducts(chatId) {
  ...
  if (!products.length) {
    return telegramApi("sendMessage", { chat_id: chatId, text: "No products are available right now." });
  }
...
async function sendTelegramPlans(chatId, productId) {
  ...
  if (!product) return telegramApi("sendMessage", { chat_id: chatId, text: "Product not found." });
...
async function createTelegramOrder(chatId, from, productId, planId) {
  ...
  await telegramApiMultipart("sendPhoto", {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: "💸 Open UPI Payment", url: `${getPublicBaseUrl()}\/telegram\/pay\/${encodeURIComponent(order.id)}` }],
...
async function sendTelegramCustomerOrders(chatId) {
  ...
  if (!orders.length) return telegramApi("sendMessage", { chat_id: chatId, text: "You don't have any Telegram orders yet." });
...
async function submitTelegramUtr(chatId, reference) {
  ...
  if (ref.length < 4 || ref.length > 100) {
    await telegramApi("sendMessage", { chat_id: chatId, text: "⚠️ Please send a valid UPI transaction ID / UTR (4–100 characters)." });
    return true;
  }
  ...
  if (!order) {
    ...
    await telegramApi("sendMessage", { chat_id: chatId, text: "Order not found. Please start a new purchase." });
    return true;
  }
  if (order.status !== "awaiting_payment") {
    telegramCustomerState.delete(String(chatId));
    await telegramApi("sendMessage", { chat_id: chatId, text: `This order is already ${order.status}.` });
    return true;
  }
  ...
  if (duplicate) {
    await telegramApi("sendMessage", { chat_id: chatId, text: "⚠️ This UPI transaction ID has already been submitted." });
    return true;
  }
...
async function handleTelegramCustomerCallback(callback) {
  ...
  if (data === "delta:menu:help") {
    ...
    await telegramApi("sendMessage", { chat_id: chatId, text: "Use /help for the DELTA.KEYS purchase steps.", parse_mode: "HTML" });
    return true;
  }
  ...
  if (planMatch) {
    ...
    try {
      ...
      await telegramApi("sendMessage", { chat_id: chatId, text: `⚠️ ${e.message || "Could not create the order."}` });
    }
    return true;
  }
  ...
  if (paidMatch) {
    ...
    await telegramApi("sendMessage", { chat_id: chatId, text: `🧾 Send the UTR / UPI transaction ID for order <code>${telegramEscape(orderId)}</code>.\n\nExample: <code>123456789012</code>\n\n/cancel to cancel.`, parse_mode: "HTML" });
    return true;
  }
  ...
  if (cancelMatch) {
    ...
    if (order && order.status === "awaiting_payment") {
      ...
      await telegramApi("sendMessage", { chat_id: chatId, text: `❌ Order <code>${telegramEscape(orderId)}</code> cancelled.`, parse_mode: "HTML" });
    } else {
...
async function handleTelegramUpdate(update) {
  ...
  if (message) {
    try { await handleTelegramMessage(message); }
    catch (e) { console.error("Telegram customer message error:", e); }
    return;
  }
  ...
  if (String(callback.data || "").startsWith("delta:") && !/^delta:(approve|reject):/.test(String(callback.data || ""))) {
    try {
      const handled = await handleTelegramCustomerCallback(callback);
      if (handled) return;
    } catch (e) {
      console.error("Telegram customer callback error:", e);
      await telegramAnswerCallback(callback.id, "Something went wrong.", true);
      return;
    }
  }
