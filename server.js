...
const TELEGRAM_ADMIN_CHAT_IDS = String(
  process.env.TELEGRAM_ADMIN_CHAT_IDS || process.env.TELEGRAM_ADMIN_CHAT_ID || ""
).split(",").map(id => id.trim()).filter(Boolean);

const TELEGRAM_PAID_CHANNEL_URL = String(
  process.env.TELEGRAM_PAID_CHANNEL_URL || "https://t.me/+b9_cXG020nUwZTc1"
).trim();
const TELEGRAM_ADMIN_USERNAME = String(
  process.env.TELEGRAM_ADMIN_USERNAME || "@DELTA_OWNER_YT"
).trim();
...
// FamPay-specific automatic verification via Gmail. No payment gateway and no LLM.
const FAMPAY_AUTO_VERIFY = String(process.env.FAMPAY_AUTO_VERIFY || "false").toLowerCase() === "true";
const FAMPAY_LOOKBACK_MINUTES = Math.max(5, Number(process.env.FAMPAY_LOOKBACK_MINUTES || 30));
const FAMPAY_POLL_MS = Math.max(10000, Number(process.env.FAMPAY_POLL_MS || 15000));
const FAMPAY_MAX_ORDERS_PER_POLL = Math.max(1, Number(process.env.FAMPAY_MAX_ORDERS_PER_POLL || 10));
let fampayVerifyRunning = false;
let fampayVerifyTimer = null;

function fampayVerifierConfigured() {
  return Boolean(FAMPAY_AUTO_VERIFY && process.env.GMAIL_CLIENT_ID && process.env.GMAIL_CLIENT_SECRET && process.env.GMAIL_REFRESH_TOKEN);
}
...
const apkUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      const safeProduct = String(req.params.id || "product").replace(/[^a-zA-Z0-9_-]/g, "_");
      const ext = path.extname(file.originalname || "").toLowerCase();
      cb(null, `${safeProduct}-${Date.now()}${ext}`);
    }
  }),
...
app.get("/admin/", (req, res) => res.sendFile(path.join(__dirname, "public", "admin-login.html")));
function addApkSidebarLink(html) {
  const script = `
<script>
(function(){
  function add(){
    if(document.getElementById('delta-apk-manager-link')) return;
    const links=[...document.querySelectorAll('a,button,[role="button"]')];
    const anchor=links.find(el=>/Telegram Bot/i.test(el.textContent||''));
    const el=document.createElement('a');
    el.id='delta-apk-manager-link'; el.href='/admin/apk'; el.textContent='📱 APK Manager';
    el.style.cssText='display:block;margin:10px 12px;padding:14px 18px;border-radius:14px;text-decoration:none;color:#cbd5e1;background:#17223b;font-weight:700;font-size:16px;border:1px solid #334155;';
    if(anchor && anchor.parentElement) anchor.parentElement.parentElement?.insertBefore(el,anchor.parentElement);
    else (document.querySelector('nav')||document.body).prepend(el);
  }
...
app.get("/admin/dashboard", adminAuth, (req, res) => {
  ...
  if(!fs.existsSync(file)) return res.status(404).send("Admin dashboard not found.");
  res.send(addApkSidebarLink(fs.readFileSync(file,"utf8")));
});
...
function ensureStore() {
  ...
  if (!fs.existsSync(DATA_FILE)) {
    const initial = {
      ...
      sidebarOrder: ["dashboard", "orders", "products", "keys", "durations", "settings", "telegram", "apk"],
      adminCredentials: {
...
function readStore() {
  ...
  for (const p of store.products || []) {
    if (!p.apk || typeof p.apk !== "object") p.apk = null;
  ...
  if (!Array.isArray(store.sidebarOrder)) {
    store.sidebarOrder = ["dashboard", "orders", "products", "keys", "durations", "settings", "telegram", "apk"];
  }
...
function adminAuth(req, res, next) {
  ...
  // Browser page navigations cannot carry the Authorization header that the
  // login form used for its API check. Keep a short-lived browser cookie too.
  if (!auth.startsWith("Basic ")) {
    const cookieAuth = getCookie(req, "delta_admin_auth");
    if (cookieAuth.startsWith("Basic ")) auth = cookieAuth;
  }
...
function telegramAdminEnabled() {
  return Boolean(TELEGRAM_ADMIN_BOT_TOKEN && TELEGRAM_ADMIN_CHAT_IDS.length);
}
...
async function telegramAnswerCallback(callbackQueryId, text, showAlert = false) {
  try {
    ...
    console.error("Telegram customer callback answer error:", e.message);
  }
}
...
async function sendTelegramCustomerMenu(chatId, text = "<b>DELTA.KEYS</b>\nChoose an option below:") {
  return telegramCustomerApi("sendMessage", {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: "🔑 Purchase Key", callback_data: "delta:menu:products" }],
        [{ text: "📋 My Keys", callback_data: "delta:menu:orders" }],
        [{ text: "🎁 Redeem Code", callback_data: "delta:menu:redeem" }],
        [{ text: "💰 Check Fund", callback_data: "delta:menu:fund" }],
        [{ text: "📚 How to Buy?", callback_data: "delta:menu:help" }],
        [{ text: "🆔 My ID", callback_data: "delta:menu:id" }],
        [{ text: "🆘 Contact Support", url: `https://t.me/${TELEGRAM_ADMIN_USERNAME.replace(/^@/, "")}` }]
      ]
    }
  });
}
...
async function telegramCustomerApiDocument(chatId, filePath, caption = "", replyMarkup = null) {
  const fileBuffer = fs.readFileSync(filePath);
  return telegramCustomerApiMultipart("sendDocument", {
    chat_id: chatId,
    caption,
    parse_mode: "HTML",
    ...(replyMarkup ? { reply_markup: replyMarkup } : {})
  }, "document", path.basename(filePath), fileBuffer, "application/vnd.android.package-archive");
}

async function sendApprovedTelegramDelivery(order, store) {
  if (!order?.customerTelegramChatId || !order?.key) return;
  const approvedProduct = (store.products || []).find(p => p.id === order.productId);
  const welcomeImage = path.join(__dirname, "delta-paid-channel-welcome.png");
  const buttons = { inline_keyboard: [
    [{ text: "🔐 JOIN PAID CHANNEL", url: TELEGRAM_PAID_CHANNEL_URL }],
    [{ text: "💬 MESSAGE ADMIN", url: `https://t.me/${TELEGRAM_ADMIN_USERNAME.replace(/^@/, "")}` }]
  ] };
  ...
  if (fs.existsSync(welcomeImage)) {
    try {
      ...
      await telegramCustomerApiMultipart("sendPhoto", {
        ...
        caption: "🎉 <b>Welcome to DELTA.KEYS Paid Channel!</b>\n\nYour payment has been verified. Your APK and key are below.",
        parse_mode: "HTML"
      }, "photo", "delta-paid-channel-welcome.png", imageBuffer, "image/png");
    } catch (e) {
      console.error("Telegram welcome image error:", e.message);
    }
  }

  const deliveryText = `🎉 <b>PAYMENT VERIFIED</b>\n\nOrder: <code>${telegramEscape(order.id)}</code>\nProduct: ${telegramEscape(order.productName)}\nDuration: ${telegramEscape(order.duration)}\n\n🔐 <b>YOUR KEY</b>\n<code>${telegramEscape(order.key)}</code>\n\n📱 Your product APK is attached when available.\n\nThank you for purchasing from <b>DELTA.KEYS</b>.`;
  if (approvedProduct?.apk?.path && fs.existsSync(approvedProduct.apk.path)) {
    await telegramCustomerApiDocument(order.customerTelegramChatId, approvedProduct.apk.path, deliveryText, buttons);
  } else {
...
async function handleTelegramCustomerCallback(callback) {
  ...
  if (data === "delta:menu:redeem") {
    await telegramAnswerCallback(callback.id, "Redeem Code selected.");
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: "🎁 <b>Redeem Code</b>\n\nRedeem-code support is not enabled on this store yet.", parse_mode: "HTML" });
    return true;
  }
  if (data === "delta:menu:fund") {
    await telegramAnswerCallback(callback.id, "Fund check selected.");
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: "💰 <b>Check Fund</b>\n\nThis store uses direct UPI payments, so no wallet balance is maintained here.", parse_mode: "HTML" });
    return true;
  }
  if (data === "delta:menu:id") {
    await telegramAnswerCallback(callback.id, "Your Telegram ID is shown.");
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: `🆔 <b>Your Telegram ID</b>\n\n<code>${telegramEscape(chatId)}</code>`, parse_mode: "HTML" });
    return true;
  }
...
async function sendTelegramPaymentApproval(order) {
  ...
  const results = [];
  for (const adminChatId of TELEGRAM_ADMIN_CHAT_IDS) {
    try {
      ...
      results.push({ chatId: adminChatId, sent: true, messageId: message.message_id });
    } catch (e) {
      console.error(`Telegram admin notification failed for ${adminChatId}:`, e.message);
      results.push({ chatId: adminChatId, sent: false, error: e.message });
    }
  }
...
async function processTelegramOrderDecision(orderId, decision) {
  ...
  if (decision === "approve") {
    ...
    if (order.customerTelegramChatId && order.key) {
      try {
        await sendApprovedTelegramDelivery(order, store);
      } catch (e) {
        console.error("Telegram customer APK/key delivery error:", e.message);
      }
    }
...
async function handleTelegramAdminUpdate(update) {
  ...
  // Only the configured admin Telegram accounts/chats may approve or reject.
  if (
    !TELEGRAM_ADMIN_CHAT_IDS.length ||
    (!TELEGRAM_ADMIN_CHAT_IDS.includes(callbackChatId) &&
      !TELEGRAM_ADMIN_CHAT_IDS.includes(callbackFromId))
  ) {
...
app.get("/admin/apk", adminAuth, (req, res) => {
  res.sendFile(path.join(__dirname, "public", "admin-apk.html"));
});

app.get("/admin/apk/download/:id", adminAuth, (req, res) => {
  const store = readStore();
  const product = store.products.find(p => p.id === req.params.id);
  if (!product?.apk?.path || !fs.existsSync(product.apk.path)) return res.status(404).send("APK not found.");
  res.download(product.apk.path, product.apk.originalName || path.basename(product.apk.path));
});

app.get("/api/admin/apks", adminAuth, (req, res) => {
  const store = readStore();
  res.json({ products: (store.products || []).map(p => ({
    id: p.id,
    name: p.name,
    apk: p.apk ? {
      originalName: p.apk.originalName || path.basename(p.apk.path || ""),
      size: Number(p.apk.size || 0),
      uploadedAt: p.apk.uploadedAt || "",
      exists: Boolean(p.apk.path && fs.existsSync(p.apk.path))
    } : null
  })) });
});

app.post("/api/admin/products/:id/apk", adminAuth, (req, res) => {
  apkUpload.single("apk")(req, res, err => {
    if (err) return res.status(400).json({ error: err.code === "LIMIT_FILE_SIZE" ? "APK is too large. Maximum size is 250 MB." : "Only .apk files are allowed." });
    const store = readStore();
    const product = store.products.find(p => p.id === req.params.id);
    if (!product) {
    ...
    if (!req.file) return res.status(400).json({ error: "Please select an APK file." });

    if (product.apk?.path && fs.existsSync(product.apk.path)) {
      try { fs.unlinkSync(product.apk.path); } catch {}
    }
    product.apk = {
      path: req.file.path,
    ...
    writeStore(store);
    res.json({ ok: true, message: `APK uploaded for ${product.name}.`, product });
  });
});

app.delete("/api/admin/products/:id/apk", adminAuth, (req, res) => {
  const store = readStore();
  const product = store.products.find(p => p.id === req.params.id);
  ...
  product.apk = null;
  writeStore(store);
  res.json({ ok: true, message: "APK removed." });
});


// ---------- Universal FamPay Gmail verifier ----------
// The customer's payment app is NOT inspected. The verifier checks the incoming
// payment notification received by the merchant's FamPay/FamApp Gmail account.
// It accepts UTR/transaction IDs from multiple common email layouts and requires
// an exact amount match before releasing a key.
const FAMPAY_ALLOWED_SENDERS = String(
  process.env.FAMPAY_ALLOWED_SENDERS || "no-reply@famapp.in"
).split(",").map(x => x.trim().toLowerCase()).filter(Boolean);
const FAMPAY_REQUIRE_SENDER_MATCH = String(
  process.env.FAMPAY_REQUIRE_SENDER_MATCH || "true"
).toLowerCase() !== "false";
const FAMPAY_EMAIL_MAX_RESULTS = Math.max(10, Math.min(100, Number(process.env.FAMPAY_EMAIL_MAX_RESULTS || 50)));
...
function decodeGmailPartBody(data) { try { return base64UrlDecode(data); } catch { return ""; } }
function collectGmailTextParts(part, out = []) {
  if (!part) return out;
  const mime = String(part.mimeType || "").toLowerCase();
  if (part.body?.data && (mime === "text/plain" || mime === "text/html" || !mime)) {
    const text = decodeGmailPartBody(part.body.data);
    if (text) out.push(mime === "text/html" ? stripHtml(text) : text);
  }
  for (const child of part.parts || []) collectGmailTextParts(child, out);
  return out;
}
...
function famPayEmailText(message) {
  return [
    ...
    ...collectGmailTextParts(message?.payload, [])
  ].join("\n").replace(/\s+/g, " ").trim();
}
...
function extractMoneyValues(text) {
  ...
  const values = [];
  const patterns = [
    /(?:₹|INR\s*|Rs\.?\s*|Rs\s*)\s*([0-9]+(?:,[0-9]{3})*(?:\.[0-9]{1,2})?)/gi,
    /(?:credited|received|payment\s+received|money\s+received|paid\s+to|sent\s+to|transaction)[^₹\d]{0,80}(?:₹|INR\s*|Rs\.?\s*)?([0-9]+(?:,[0-9]{3})*(?:\.[0-9]{1,2})?)/gi
  ];
...
function extractReferenceCandidates(text) {
  ...
  const refs = new Set();
  const patterns = [
    /\bUTR\s*(?:number|no\.?|id)?\s*[:#-]?\s*([A-Z0-9]{8,40})\b/gi,
    /\bUPI\s*(?:reference|ref(?:erence)?|transaction|txn)\s*(?:number|no\.?|id)?\s*[:#-]?\s*([A-Z0-9]{8,40})\b/gi,
    /\b(?:Transaction|Txn|Reference|Ref)\s*(?:ID|Id|No\.?|Number)?\s*[:#-]?\s*([A-Z0-9]{8,40})\b/gi
  ];
  for (const re of patterns) {
    let m;
    while ((m = re.exec(source))) refs.add(m[1].trim());
  }
  return [...refs];
}
function extractFamPayReference(text) {
  const refs = extractReferenceCandidates(text);
  const utr = refs.find(x => /^\d{12}$/.test(x)) || "";
  const transactionId = refs.find(x => x !== utr) || "";
  return { utr, transactionId, candidates: refs };
}
function senderMatchesFamPay(fromHeader) {
  const from = String(fromHeader || "").toLowerCase();
  if (!FAMPAY_REQUIRE_SENDER_MATCH) return true;
  if (FAMPAY_ALLOWED_SENDERS.some(x => from.includes(x))) return true;
  // Allow future FamApp notification addresses under the official domain.
  return /@[a-z0-9.-]*famapp\.in\b/i.test(from);
}
function looksLikeIncomingUpiPayment(text) {
  const s = String(text || "").toLowerCase();
  const paymentWords = [
    "upi", "payment received", "money received", "received money", "credited",
    "transaction successful", "payment successful", "amount received", "utr"
  ];
  return paymentWords.some(word => s.includes(word));
}
function emailTimestampMs(message) {
  const date = headerValue(message?.payload?.headers, "Date");
  const t = Date.parse(date);
  return Number.isFinite(t) ? t : 0;
}
async function gmailRefreshAccessToken() {
  ...
  if (!response.ok || !data.access_token) {
    throw new Error(data.error_description || data.error || `Gmail OAuth failed (${response.status}).`);
  }
  return data.access_token;
}
async function gmailApi(pathname, accessToken, params) {
...
async function listFamPayCandidateMessages(accessToken, order) {
  const days = Math.max(1, Math.ceil(FAMPAY_LOOKBACK_MINUTES / 1440));
  const ref = String(order.paymentReference || "").trim();
  const queries = [];
  if (ref) queries.push(`"${ref}" newer_than:${days}d`);
  // These fallback queries make the verifier independent of the customer's UPI app.
  queries.push(`newer_than:${days}d (upi OR credited OR "payment received" OR "money received")`);
  queries.push(`newer_than:${days}d from:(famapp.in)`);

  const ids = new Set();
  for (const q of queries) {
    const list = await gmailApi("messages", accessToken, { q, maxResults: FAMPAY_EMAIL_MAX_RESULTS });
    for (const item of list.messages || []) {
      if (item?.id) ids.add(item.id);
      if (ids.size >= FAMPAY_EMAIL_MAX_RESULTS) break;
    }
    if (ids.size >= FAMPAY_EMAIL_MAX_RESULTS) break;
  }
  return [...ids];
}
async function findFamPayEmailForOrder(order, accessToken) {
  const ref = String(order.paymentReference || "").trim().toLowerCase();
  const expected = normalizeMoney(order.amount);
  const ids = await listFamPayCandidateMessages(accessToken, order);
  const candidates = [];

  for (const id of ids) {
    const message = await gmailApi(`messages/${encodeURIComponent(id)}`, accessToken, { format: "full" });
    const from = headerValue(message?.payload?.headers, "From");
    const text = famPayEmailText(message);
    const refs = extractFamPayReference(text);
    const moneyValues = extractMoneyValues(text);
    const refPresent = ref && (
      text.toLowerCase().includes(ref) ||
      refs.candidates.some(x => x.toLowerCase() === ref)
    );
    const amountPresent = Number.isFinite(expected) && moneyValues.includes(expected);
    const senderOk = senderMatchesFamPay(from);
    const paymentLike = looksLikeIncomingUpiPayment(text);

    if (!senderOk || !paymentLike || !amountPresent || !refPresent) continue;
    ...
    candidates.push({
      ...
      refs,
      moneyValues,
      emailSubject: headerValue(message?.payload?.headers, "Subject"),
      emailDate: headerValue(message?.payload?.headers, "Date"),
      timestamp: emailTimestampMs(message)
    });
  }
...
async function verifyFamPayOrder(order) {
  const accessToken = await gmailRefreshAccessToken();
  const found = await findFamPayEmailForOrder(order, accessToken);
  if (!found) {
    return {
      verified: false,
      reason: "No matching FamPay/FamApp incoming UPI email was found with the submitted UTR/transaction ID and exact order amount."
    };
  }
  return {
    verified: true,
    refs: found.refs,
    matchedAmount: normalizeMoney(order.amount),
    emailSubject: found.emailSubject,
    emailDate: found.emailDate
  };
}
async function autoVerifyFamPayOrders() {
  if (!fampayVerifierConfigured() || fampayVerifyRunning) return;
  fampayVerifyRunning = true;
  try {
    const store = readStore();
    const now = Date.now();
    const candidates = (store.orders || [])
      .filter(o => o.status === "pending" && o.paymentReference && o.paymentStartedAt)
      .filter(o => now - new Date(o.paymentStartedAt).getTime() <= FAMPAY_LOOKBACK_MINUTES * 60000)
      .slice(0, FAMPAY_MAX_ORDERS_PER_POLL);

    for (const order of candidates) {
      try {
        const result = await verifyFamPayOrder(order);
        if (!result.verified) {
          console.log(`FamPay universal auto-check ${order.id}: ${result.reason}`);
          continue;
        }

        const freshStore = readStore();
        const freshOrder = freshStore.orders.find(o => o.id === order.id);
        if (!freshOrder || freshOrder.status !== "pending") continue;
        if (String(freshOrder.paymentReference || "").toLowerCase() !== String(order.paymentReference || "").toLowerCase()) continue;

        fulfillPaidOrder(freshStore, freshOrder, freshOrder.paymentReference);
        freshOrder.approvedVia = "fampay-gmail-universal-auto";
        freshOrder.approvedAt = new Date().toISOString();
        freshOrder.paymentVerification = {
          method: "fampay-gmail",
          matchedAmount: result.matchedAmount,
          utr: result.refs?.utr || "",
          transactionId: result.refs?.transactionId || "",
          emailSubject: result.emailSubject || "",
          emailDate: result.emailDate || "",
          verifiedAt: freshOrder.approvedAt
        };
        writeStore(freshStore);

        if (freshOrder.customerTelegramChatId && freshOrder.key) {
          try {
            await sendApprovedTelegramDelivery(freshOrder, freshStore);
          } catch (e) {
            console.error(`FamPay APK/key delivery failed for ${freshOrder.id}:`, e.message);
          }
        }
        console.log(`DELTA.KEYS universal FamPay payment verified: ${freshOrder.id} / ₹${freshOrder.amount} / UTR ${freshOrder.paymentReference}`);
      } catch (e) {
        console.error(`FamPay universal auto verification failed for ${order.id}:`, e.message);
      }
    }
  } finally {
    fampayVerifyRunning = false;
  }
}
function startFamPayVerifier() {
  if (!fampayVerifierConfigured()) {
    if (FAMPAY_AUTO_VERIFY) console.log("FamPay universal auto verification is enabled but Gmail OAuth variables are incomplete.");
    return;
  }
  console.log(`DELTA.KEYS universal FamPay Gmail verification enabled (poll ${FAMPAY_POLL_MS}ms).`);
  autoVerifyFamPayOrders().catch(e => console.error("FamPay startup check failed:", e.message));
  fampayVerifyTimer = setInterval(() => autoVerifyFamPayOrders().catch(e => console.error("FamPay poll error:", e.message)), FAMPAY_POLL_MS);
}
...
function shutdown(signal) {
  ...
  stopTelegramPolling();
  if (fampayVerifyTimer) clearInterval(fampayVerifyTimer);
