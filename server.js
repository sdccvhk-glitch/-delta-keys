...
const TELEGRAM_WELCOME_IMAGE_PATH = path.join(
  __dirname,
  "delta-paid-channel-welcome.png"
);
...
async function sendTelegramCustomerMenu(chatId, text = "<b>Welcome to DELTA.KEYS</b> 🔑\n\nSelect an option below:") {
  return telegramCustomerApi("sendMessage", {
    ...
    reply_markup: {
      keyboard: [
        [{ text: "🔑 Purchase Key" }],
        [{ text: "📋 My Keys" }],
        [{ text: "🎁 Redeem Code" }, { text: "💰 Check Fund" }],
        [{ text: "📚 How to Buy?" }],
        [{ text: "🆔 My ID" }, { text: "🆘 Contact Support" }]
      ],
...
async function handleTelegramMessage(message) {
  ...
  // Persistent customer menu buttons (matching the old bot menu layout).
  if (text === "🔑 Purchase Key" || text === "Purchase Key") {
    telegramCustomerState.delete(chatId);
    await sendTelegramProducts(chatId);
    return;
  }
  if (text === "📋 My Keys" || text === "My Keys") {
    telegramCustomerState.delete(chatId);
    await sendTelegramCustomerOrders(chatId);
    return;
  }
  if (text === "🎁 Redeem Code" || text === "Redeem Code") {
    telegramCustomerState.delete(chatId);
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: "🎁 <b>REDEEM CODE</b>\n\nSend your redeem code in the chat.\n\nThis feature is not configured yet. Please contact support if you received a code.", parse_mode: "HTML" });
    return;
  }
  if (text === "💰 Check Fund" || text === "Check Fund") {
    telegramCustomerState.delete(chatId);
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: "💰 <b>CHECK FUND</b>\n\nYour balance/fund wallet is not enabled for DELTA.KEYS yet. Purchases currently use UPI payment.", parse_mode: "HTML" });
    return;
  }
  if (text === "📚 How to Buy?" || text === "How to Buy?") {
    telegramCustomerState.delete(chatId);
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: "📚 <b>HOW TO BUY</b>\n\n1. Tap <b>Purchase Key</b>.\n2. Select your product.\n3. Select the duration.\n4. Pay the exact UPI amount shown.\n5. Tap <b>I Have Paid</b> and send your UTR.\n6. Admin verifies the payment.\n7. After approval, your APK and key are delivered here.", parse_mode: "HTML" });
    return;
  }
  if (text === "🆔 My ID" || text === "My ID") {
    telegramCustomerState.delete(chatId);
    const username = from?.username ? `@${telegramEscape(from.username)}` : "Not set";
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: `🆔 <b>MY ID</b>\n\n<b>Telegram ID:</b> <code>${telegramEscape(chatId)}</code>\n<b>Username:</b> ${username}`, parse_mode: "HTML" });
    return;
  }
  if (text === "🆘 Contact Support" || text === "Contact Support") {
    telegramCustomerState.delete(chatId);
    await telegramCustomerApi("sendMessage", { chat_id: chatId, text: `🆘 <b>CONTACT SUPPORT</b>\n\nMessage the admin: <b>${telegramEscape(TELEGRAM_ADMIN_USERNAME)}</b>`, parse_mode: "HTML", reply_markup: { inline_keyboard: [[{ text: "💬 MESSAGE ADMIN", url: `https://t.me/${TELEGRAM_ADMIN_USERNAME.replace(/^@/, "")}` }]] } });
    return;
  }
  if (text === "/help") {
...
async function processTelegramOrderDecision(orderId, decision) {
  ...
  if (decision === "approve") {
    ...
    if (order.customerTelegramChatId && order.key) {
      try {
        const customerChatId = order.customerTelegramChatId;
        const approvedProduct = store.products.find(p => p.id === order.productId);
        const joinMarkup = { inline_keyboard: [
          [{ text: "🔐 JOIN 4.6 PAID CHANNEL", url: TELEGRAM_PAID_CHANNEL_URL }]
        ] };
        const deliveryMarkup = { inline_keyboard: [
          [{ text: "🔐 JOIN 4.6 PAID CHANNEL", url: TELEGRAM_PAID_CHANNEL_URL }],
          [{ text: "💬 MESSAGE ADMIN", url: `https://t.me/${TELEGRAM_ADMIN_USERNAME.replace(/^@/, "")}` }]
        ] };

        // Send the paid-channel welcome image first.
        if (fs.existsSync(TELEGRAM_WELCOME_IMAGE_PATH)) {
          await telegramCustomerApiMultipart("sendPhoto", {
            chat_id: customerChatId,
            caption: `🎉 <b>PAYMENT APPROVED</b>\n\n<b>Welcome to our 4.6 PAID CHANNEL!</b>\nYour order has been approved. Your APK and key are below.`,
            parse_mode: "HTML",
            reply_markup: joinMarkup
          }, "photo", "delta-paid-channel-welcome.png", fs.readFileSync(TELEGRAM_WELCOME_IMAGE_PATH), "image/png");
        } else {
          await telegramCustomerApi("sendMessage", {
            chat_id: customerChatId,
        ...
        const apkPath = approvedProduct?.apk?.path;
        if (apkPath && fs.existsSync(apkPath)) {
          // APK + key are deliberately delivered in ONE Telegram document message.
          await telegramCustomerApiDocument(
            customerChatId,
            apkPath,
            `📱 <b>${telegramEscape(order.productName)} APK</b>\n\n<b>Order:</b> <code>${telegramEscape(order.id)}</code>\n<b>Duration:</b> ${telegramEscape(order.duration)}\n\n🔐 <b>YOUR KEY</b>\n<code>${telegramEscape(order.key)}</code>\n\nThank you for purchasing from <b>DELTA.KEYS</b>.`,
            deliveryMarkup
          );
        } else {
          await telegramCustomerApi("sendMessage", {
