...
async function sendTelegramCustomerMenu(chatId, text = "") {
  const menuText = text || (
    `<b>💜⚡ DELTA.KEYS ⚡💜</b>\n\n` +
    `<b>PREMIUM DIGITAL KEY STORE</b> 🔑\n\n` +
    `👋 Welcome to <b>DELTA.KEYS</b>!\n` +
    `Your trusted source for premium digital keys.\n\n` +
    `⚡ <b>Fast Delivery</b>   🛡️ <b>Verified Orders</b>\n` +
    `💎 <b>Premium Keys</b>   🔐 <b>Secure Payment</b>\n\n` +
    `<i>🚀 Ready to get started? Choose an option below.</i>`
  );
  ...
  return telegramCustomerApi("sendMessage", {
    ...
    reply_markup: {
      inline_keyboard: [
        [telegramButton("🔑  BUY KEYS  ›", "delta:menu:products"), telegramButton("📦  MY ORDERS  ›", "delta:menu:orders")],
        [telegramButton("❓  HELP & SUPPORT", "delta:menu:help")]
      ]
    }
  });
}
...
async function sendTelegramProducts(chatId) {
  ...
  products.forEach((product, index) => {
    ...
    rows.push([
      telegramButton(
        `${icon} ${product.name.replace(/^DELTA\s*/i, "")}  •  ${stockText}`,
        `delta:product:${telegramShortId(product.id)}`
      )
    ]);
  });

  rows.push([
    telegramButton("‹ BACK TO MENU", "delta:menu:back")
  ]);

  return telegramCustomerApi("sendMessage", {
    chat_id: chatId,
    text:
      `<b>💜⚡ DELTA.KEYS STORE ⚡💜</b>\n\n` +
      `<b>🔑 SELECT YOUR KEY</b>\n` +
      `━━━━━━━━━━━━━━━━━━\n` +
      `💎 Premium digital access\n` +
      `⚡ Fast delivery after verification\n` +
      `🛡️ Admin-verified orders\n\n` +
      `<i>Choose a product below to view its durations and exact prices.</i>`,
    parse_mode: "HTML",
...
async function sendTelegramPlans(chatId, productId) {
  ...
  if (!plans.length) {
    return telegramCustomerApi("sendMessage", {
      chat_id: chatId,
      text: `<b>💜 ${telegramEscape(product.name)}</b>\n\n⚠️ No durations are available.`,
      parse_mode: "HTML"
    });
  }

  const rows = plans.map((plan, index) => {
    const icon = index === plans.length - 1 ? "👑" : "⚡";
    return [
      telegramButton(
        `${icon} ${plan.name}   •   ₹${Number(plan.price || 0).toFixed(2)}`,
        `delta:plan:${telegramShortId(product.id)}:${telegramShortId(plan.id)}`
      )
    ];
  });

  rows.push([
    telegramButton("‹ BACK TO PRODUCTS", "delta:menu:products")
  ]);
  ...
  return telegramCustomerApi("sendMessage", {
    ...
    text:
      ...
      `${telegramEscape(product.description || "Premium digital access key.")}\n\n` +
      `🟢 <b>Stock:</b> ${stock} available\n` +
      `💳 <b>Payment:</b> UPI\n\n` +
      `<b>⚡ CHOOSE YOUR DURATION</b>\n` +
      `<i>Select a plan to continue to the exact-price payment screen.</i>`,
    parse_mode: "HTML",
    reply_markup: { inline_keyboard: rows }
  });
}
...
async function handleTelegramMessage(message) {
  ...
  if (text === "/start" || text === "/menu") {
    ...
    await sendTelegramCustomerMenu(
      ...
      `Your trusted source for premium keys, fast delivery and secure transactions.\n\n` +
      `⚡ <b>Fast Delivery</b>\n` +
      `🛡️ <b>Admin-Verified Orders</b>\n` +
      `💎 <b>Premium Digital Access</b>\n` +
...
async function handleTelegramCustomerCallback(callback) {
  ...
  if (data === "delta:menu:back") {
    await telegramAnswerCallback(callback.id, "Back to menu");
    await sendTelegramCustomerMenu(chatId);
    return true;
  }
