    ...
    for (const order of candidates) {
      try {
        ...
        if (freshOrder.customerTelegramChatId && freshOrder.key) {
          try {
            await telegramCustomerApi("sendMessage", {
              chat_id: freshOrder.customerTelegramChatId,
              text: `🎉 <b>PAYMENT VERIFIED AUTOMATICALLY</b>\n\nOrder: <code>${telegramEscape(freshOrder.id)}</code>\nProduct: ${telegramEscape(freshOrder.productName)}\nDuration: ${telegramEscape(freshOrder.duration)}\nAmount: ₹${Number(freshOrder.amount || 0).toFixed(2)}\n\n🔑 <b>Your Key:</b>\n<code>${telegramEscape(freshOrder.key)}</code>\n\nThank you for purchasing from <b>DELTA.KEYS</b>.`,
              parse_mode: "HTML"
            });
          } catch (e) {
            console.error(`FamPay key delivery failed for ${freshOrder.id}:`, e.message);
          }
        }
