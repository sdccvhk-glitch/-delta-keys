...
app.get("/admin", (req, res) => { res.set("Cache-Control", "no-store"); res.sendFile(path.join(__dirname, "public", "admin-login.html")); });
app.get("/admin/", (req, res) => { res.set("Cache-Control", "no-store"); res.sendFile(path.join(__dirname, "public", "admin-login.html")); });
...
async function telegramCustomerApiDocument(chatId, filePath, caption = "") {
      ...
      try {
        ...
        if (approvedProduct?.apk?.path && fs.existsSync(approvedProduct.apk.path)) {
          await telegramCustomerApiDocument(
            ...
            approvedProduct.apk.path,
            `📱 <b>${telegramEscape(order.productName)}</b> APK\n\nOrder: <code>${telegramEscape(order.id)}</code>\nDuration: ${telegramEscape(order.duration)}\n\nYour product APK is attached.`
          );
        }
