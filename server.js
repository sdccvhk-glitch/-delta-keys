...
// Option A: verify real bank-credit alert emails and auto-release the key.
// This does not use Razorpay, PayU, Cashfree, or another payment gateway.
const AUTO_PAYMENT_VERIFY = String(process.env.AUTO_PAYMENT_VERIFY || "false").toLowerCase() === "true";
const AUTO_PAYMENT_LOOKBACK_MINUTES = Math.max(5, Number(process.env.AUTO_PAYMENT_LOOKBACK_MINUTES || 30));
const AUTO_PAYMENT_POLL_MS = Math.max(10000, Number(process.env.AUTO_PAYMENT_POLL_MS || 15000));
const AUTO_PAYMENT_MAX_ORDERS_PER_POLL = Math.max(1, Number(process.env.AUTO_PAYMENT_MAX_ORDERS_PER_POLL || 5));
let autoPaymentVerifyTimer = null;
let autoPaymentVerifyRunning = false;
const autoPaymentSeenMessageIds = new Set();
let upiAgentModulePromise = null;
...
const apkUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      cb(null, `${safeProduct}-${Date.now()}-${crypto.randomBytes(4).toString("hex")}.apk`);
    }
  }),
  limits: { fileSize: 250 * 1024 * 1024 },
  fileFilter: (_req, file, cb) => {
    const ext = path.extname(file.originalname || "").toLowerCase();
    if (ext !== ".apk") return cb(new Error("ONLY_APK"));
    cb(null, true);
  }
});
...
function autoPaymentVerifierConfigured() {
  return Boolean(
    AUTO_PAYMENT_VERIFY &&
...
async function getUpiAgent() {
  if (!upiAgentModulePromise) upiAgentModulePromise = import("upiagent");
  return upiAgentModulePromise;
}

function getAutoPaymentLlmConfig() {
  if (process.env.OPENAI_API_KEY) return { provider: "openai", model: process.env.UPIAGENT_OPENAI_MODEL || "gpt-4o-mini", apiKey: process.env.OPENAI_API_KEY };
  if (process.env.ANTHROPIC_API_KEY) return { provider: "anthropic", model: process.env.UPIAGENT_ANTHROPIC_MODEL || "claude-sonnet-4-5-20250514", apiKey: process.env.ANTHROPIC_API_KEY };
  if (process.env.GOOGLE_GENERATIVE_AI_API_KEY) return { provider: "gemini", model: process.env.UPIAGENT_GEMINI_MODEL || "gemini-2.0-flash", apiKey: process.env.GOOGLE_GENERATIVE_AI_API_KEY };
  if (process.env.OPENROUTER_API_KEY) return { provider: "openrouter", model: process.env.UPIAGENT_OPENROUTER_MODEL || "openai/gpt-4o-mini", apiKey: process.env.OPENROUTER_API_KEY };
  return null;
}

async function sendTelegramAutoPaymentNotice(order, payment) {
  if (!telegramAdminEnabled()) return;
  for (const adminChatId of TELEGRAM_ADMIN_CHAT_IDS) {
    try {
      await telegramAdminApi("sendMessage", {
        chat_id: adminChatId,
        text: [
          ...
          `<b>UTR:</b> <code>${telegramEscape(payment?.upiReferenceId || order.paymentReference || "")}</code>`,
          `<b>Bank:</b> ${telegramEscape(payment?.bankName || "")}`,
          "", "Key released automatically after bank-alert verification."
        ].join("\n"),
        parse_mode: "HTML"
      });
    } catch (e) {
      console.error(`Telegram auto-payment notice failed for ${adminChatId}:`, e.message);
    }
  }
}

async function deliverTelegramPaidOrder(order) {
  if (!order.customerTelegramChatId || !order.key || !telegramCustomerEnabled()) return;
  try {
    ...
    console.error("Telegram automatic key delivery error:", e.message);
  }
}

async function autoVerifyPendingOrders() {
  if (!autoPaymentVerifierConfigured() || autoPaymentVerifyRunning) return;
  autoPaymentVerifyRunning = true;
  try {
    const { fetchAndVerifyPayment } = await getUpiAgent();
    const store = readStore();
    const now = Date.now();
    const candidates = (store.orders || [])
      .filter(o => (o.status === "awaiting_payment" || o.status === "pending") && o.paymentStartedAt)
      .filter(o => now - new Date(o.paymentStartedAt).getTime() <= AUTO_PAYMENT_LOOKBACK_MINUTES * 60000)
      .slice(0, AUTO_PAYMENT_MAX_ORDERS_PER_POLL);

    for (const order of candidates) {
      const sameAmountOpen = candidates.filter(x => x.id !== order.id && Number(x.amount) === Number(order.amount));
      if (sameAmountOpen.length && !order.paymentReference) continue;
      try {
        const result = await fetchAndVerifyPayment({
          ...
          llm: getAutoPaymentLlmConfig(),
          expected: {
            amount: Number(order.amount),
            timeWindowMinutes: AUTO_PAYMENT_LOOKBACK_MINUTES,
            amountTolerancePercent: 0
          },
          lookbackMinutes: AUTO_PAYMENT_LOOKBACK_MINUTES,
          maxEmails: 10,
          skipMessageIds: autoPaymentSeenMessageIds,
          expectedUtrs: order.paymentReference ? [order.paymentReference] : undefined
        });
        for (const id of result.processedMessageIds || []) autoPaymentSeenMessageIds.add(id);
        if (!result.verified || !result.payment?.upiReferenceId) continue;

        const freshStore = readStore();
        const freshOrder = freshStore.orders.find(o => o.id === order.id);
        if (!freshOrder || freshOrder.status === "paid" || freshOrder.status === "rejected") continue;
        if (Number(result.payment.amount) !== Number(freshOrder.amount)) continue;

        fulfillPaidOrder(freshStore, freshOrder, result.payment.upiReferenceId);
        freshOrder.approvedVia = "auto-bank-alert";
        ...
        await deliverTelegramPaidOrder(freshOrder);
        await sendTelegramAutoPaymentNotice(freshOrder, result.payment);
        console.log(`DELTA.KEYS auto payment verified: ${freshOrder.id} / ₹${freshOrder.amount}`);
      } catch (e) {
        console.error(`Auto payment verification failed for ${order.id}:`, e.message);
      }
    }
  } catch (e) {
    console.error("Auto payment verifier error:", e.message);
  } finally {
    autoPaymentVerifyRunning = false;
  }
}

function startAutoPaymentVerifier() {
  if (!autoPaymentVerifierConfigured()) {
    if (AUTO_PAYMENT_VERIFY) console.log("Option A auto-payment verification is enabled but Gmail/LLM variables are incomplete.");
    return;
  }
  console.log(`DELTA.KEYS Option A bank-alert verification enabled (poll ${AUTO_PAYMENT_POLL_MS}ms).`);
  autoVerifyPendingOrders().catch(() => {});
  autoPaymentVerifyTimer = setInterval(() => autoVerifyPendingOrders().catch(() => {}), AUTO_PAYMENT_POLL_MS);
}

function stopAutoPaymentVerifier() {
  if (autoPaymentVerifyTimer) clearInterval(autoPaymentVerifyTimer);
  autoPaymentVerifyTimer = null;
}
...
async function submitTelegramUtr(chatId, reference) {
  ...
  await telegramCustomerApi("sendMessage", {
    chat_id: chatId,
    text: `<b>✅ PAYMENT DETAILS RECEIVED</b>\n\nOrder: <code>${telegramEscape(order.id)}</code>\nUTR: <code>${telegramEscape(ref)}</code>\n\nThe system is checking the bank payment alert automatically. Your key will be sent here after verification.`,
    parse_mode: "HTML"
  });
...
app.get("/api/health", (req, res) =>
  res.json({
    ok: true,
    app: "DELTA.KEYS",
    paymentMode: AUTO_PAYMENT_VERIFY ? "upi-exact-amount-bank-alert-auto" : "manual-upi-admin-approval",
    autoPaymentVerification: autoPaymentVerifierConfigured(),
...
app.post("/api/payments/submit-utr", async (req, res) => {
  try {
    ...
    res.json({
      order: publicOrder(order),
      message: AUTO_PAYMENT_VERIFY
        ? "UTR submitted. The system will verify the bank credit automatically."
...
app.get("/admin/apk", (req, res) => {
  res.redirect(302, "/admin/dashboard?view=apk");
});
  ...
  apkUpload.single("apk")(req, res, err => {
    if (err) {
      ...
      if (err.code === "LIMIT_FILE_SIZE") {
        return res.status(400).json({ error: "APK is too large. Maximum size is 250 MB." });
      }
...
const server = app.listen(PORT, "0.0.0.0", () => {
  ...
  startAutoPaymentVerifier();
});

function shutdown(signal) {
  console.log(`${signal} received. Shutting down...`);
  stopTelegramPolling();
  stopAutoPaymentVerifier();
