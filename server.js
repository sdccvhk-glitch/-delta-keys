...
const apkUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const ok = String(file.originalname || "").toLowerCase().endsWith(".apk");
...
app.use("/uploads/apks", express.static(APK_DIR));

app.get("/admin/apk", (req, res) => res.sendFile(path.join(__dirname, "public", "admin-apk.html")));
...
app.get("/api/admin/apks", adminAuth, (req, res) => {
  ...
  res.json({ products: (store.products || []).map(p => ({
    id: p.id, name: p.name, apk: p.apk || null
  })) });
});

app.post("/api/admin/products/:id/apk", adminAuth, apkUpload.single("apk"), (req, res) => {
  try {
    const store = readStore();
    const product = store.products.find(p => p.id === req.params.id);
    if (!product) return res.status(404).json({ error: "Product not found." });
    if (!req.file) return res.status(400).json({ error: "Select an APK file." });

    if (product.apk?.filename) {
      const oldPath = path.join(APK_DIR, product.apk.filename);
    ...
    product.apk = {
      ...
      size: req.file.size,
      url: `/uploads/apks/${encodeURIComponent(req.file.filename)}`,
    ...
    res.status(400).json({ error: e.message || "Could not upload APK." });
  }
});
