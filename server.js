...
async function handleTelegramMessage(message) {
  ...
  if (text === "/start" || text === "/menu") {
    ...
    if (fs.existsSync(TELEGRAM_WELCOME_IMAGE_PATH)) {
      try {
        await telegramCustomerApiMultipart(
          "sendPhoto",
          {
            chat_id: chatId,
            caption:
              "💜 <b>WELCOME TO DELTA.KEYS</b>\n\n" +
              "🔑 Premium digital keys\n" +
              "⚡ Fast Telegram delivery after admin verification\n" +
              "💳 Exact-price UPI payment\n\n" +
              "Choose an option below:",
            parse_mode: "HTML"
          },
          "photo",
          "delta-paid-channel-welcome.png",
          fs.readFileSync(TELEGRAM_WELCOME_IMAGE_PATH),
          "image/png"
        );
      } catch (e) {
    ...
    await sendTelegramCustomerMenu(
      chatId,
      `<b>DELTA.KEYS</b> 🔑\n\n` +
  ...
  if (text === "💰 Check Fund") {
    await telegramCustomerApi("sendMessage", {
      chat_id: chatId,
      text: "💰 <b>Check Fund</b>\n\nNo wallet/fund balance is connected to this bot.",
      parse_mode: "HTML"
    });
    return;
  }
  ...
  if (text === "📚 How to Buy?") {
    await telegramCustomerApi("sendMessage", {
      ...
      text:
        "📚 <b>HOW TO BUY</b>\n\n" +
        "1️⃣ Select <b>Purchase Key</b>.\n" +
        "2️⃣ Choose your application/key.\n" +
        "3️⃣ Choose the duration and price.\n" +
        "4️⃣ Pay the exact amount using UPI.\n" +
        "5️⃣ Tap <b>I Have Paid</b> and send your UTR.\n" +
        "6️⃣ Admin verifies the payment.\n" +
        "7️⃣ Your key is delivered in a separate message.",
      parse_mode: "HTML"
    });
    return;
  }
  ...
  if (text === "🆔 My ID") {
    await telegramCustomerApi("sendMessage", {
      chat_id: chatId,
      text: `🆔 <b>Your Telegram ID</b>\n\n<code>${telegramEscape(chatId)}</code>`,
      parse_mode: "HTML"
    });
    return;
  }
  ...
  if (text === "🆘 Contact Support") {
    await telegramCustomerApi("sendMessage", {
      chat_id: chatId,
      text: "🆘 <b>DELTA.KEYS SUPPORT</b>\n\nFor payment, key, or channel-access issues, message the admin.",
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [[
          { text: "💬 MESSAGE ADMIN", url: "https://t.me/DELTA_OWNER_YT" }
        ]]
      }
    });
    return;
  ...
  if (decision === "approve") {
    ...
    if (order.customerTelegramChatId && order.key) {
      try {
          ...
          await telegramCustomerApiMultipart(
            "sendPhoto",
            {
              ...
              caption:
                `💜 <b>WELCOME TO DELTA.KEYS</b>\n\n` +
                `🎉 Payment approved!\n` +
                `🔐 Welcome to our <b>4.6 PAID CHANNEL</b>.`,
              parse_mode: "HTML"
            },
            "photo",
            "delta-paid-channel-welcome.png",
        ...
        const adminUrl = "https://t.me/DELTA_OWNER_YT";

        // The key is intentionally delivered in a separate message.
        await telegramCustomerApi("sendMessage", {
          chat_id: order.customerTelegramChatId,
          text:
            `🔑 <b>YOUR KEY</b>\n\n` +
            ...
            `🧾 Order: <code>${telegramEscape(order.id)}</code>\n` +
            `📦 ${telegramEscape(order.productName)}\n` +
            `⏱️ ${telegramEscape(order.duration)}\n\n` +
            `💜 Welcome to our paid channel!\n` +
            `If you have any issue with your key or channel access, message admin.\n\n` +
            `✨ <b>DELTA.KEYS</b>`,
          parse_mode: "HTML",
          reply_markup: {
            inline_keyboard: [
              [{ text: "🔐  JOIN 4.6 PAID CHANNEL", url: TELEGRAM_PAID_CHANNEL_URL }],
              [{ text: "💬  MESSAGE ADMIN", url: adminUrl }]
            ]
          }
        });
      } catch (e) {
