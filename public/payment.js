...
const form=document.getElementById('paymentForm'),startButton=document.getElementById('startPaymentButton'),submitButton=document.getElementById('submitUtrButton'),openUpiButton=document.getElementById('openUpiButton'),paidConfirm=document.getElementById('paidConfirm');
...
startButton.addEventListener('click',async()=>{
 ...
 try{
  ...
  if(out.qrDataUrl){ upiQr.src=out.qrDataUrl; upiQr.style.display='block'; }
  ...
  if(out.upiLink){
    ...
    openUpiButton.hidden=false;
    startButton.hidden=true;
  }
  paymentStep.querySelector('.muted').textContent='Scan the QR above or tap Open UPI App. The QR and UPI link contain the exact order amount shown above.';
...
paidConfirm.addEventListener('change',()=>{ submitButton.disabled=!paidConfirm.checked; });

form.addEventListener('submit',async e=>{
 e.preventDefault(); if(!order?.id){error.textContent='Generate the payment QR first.';return;}
 if(!paidConfirm.checked){error.textContent='Please confirm that you completed the UPI payment first.';return;}
