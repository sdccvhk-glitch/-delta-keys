...
async function api(url,opt={}){
  ...
  opt.headers={...(opt.headers||{}),Authorization:auth};
...
async function loadAdmin(){
  ...
  renderStock(products,pool);renderProducts(products);renderPlans(products);renderOrders(orders);renderApks(products);refreshUploadPlans();
}
...
function formatApkSize(n){n=Number(n||0);if(!n)return '—';const u=['B','KB','MB','GB'];let i=0;while(n>=1024&&i<3){n/=1024;i++}return `${n.toFixed(i?1:0)} ${u[i]}`}
function renderApks(products){
  const box=$('apkManagerList');if(!box)return;
  if(!products.length){box.innerHTML='<p class="video-muted">No products found. Add a product first.</p>';return;}
  box.innerHTML=products.map(p=>{
    const a=p.apk;
    const attached=Boolean(a&&a.exists);
    return `<article class="order-card apk-card">
      <div class="order-top"><div><h3>📱 ${esc(p.name)}</h3><div class="order-meta">Product ID: ${esc(p.id)}</div></div><span class="video-chip ${attached?'success':''}">${attached?'APK ATTACHED':'NO APK'}</span></div>
      <div class="order-meta" style="margin-top:10px">${attached?`Current APK: <b>${esc(a.originalName||'app.apk')}</b> • ${formatApkSize(a.size)}`:'No APK is attached to this product yet.'}</div>
      <div class="admin-controls" style="margin-top:14px;display:flex;gap:10px;flex-wrap:wrap;align-items:center">
        <input id="apk-file-${escAttr(p.id)}" type="file" accept=".apk,application/vnd.android.package-archive" class="video-input" style="flex:1;min-width:220px">
        <button class="video-primary" type="button" onclick="uploadProductApk('${escAttr(p.id)}')">${attached?'Replace APK':'Upload APK'}</button>
        ${attached?`<button class="danger-btn" type="button" onclick="removeProductApk('${escAttr(p.id)}')">Remove APK</button>`:''}
      </div>
      <div class="order-meta" style="margin-top:8px">Maximum size: 250 MB • Only .apk files are accepted.</div>
    </article>`;
  }).join('');
}
async function uploadProductApk(id){
  const input=$('apk-file-'+id);if(!input||!input.files[0])return alert('Select an APK file first.');
  const fd=new FormData();fd.append('apk',input.files[0]);
  try{await api('/api/admin/products/'+encodeURIComponent(id)+'/apk',{method:'POST',body:fd});alert('APK uploaded successfully.');await loadAdmin()}catch(e){alert(e.message)}}
async function removeProductApk(id){
  if(!confirm('Remove the APK attached to this product?'))return;
  try{await api('/api/admin/products/'+encodeURIComponent(id)+'/apk',{method:'DELETE'});alert('APK removed.');await loadAdmin()}catch(e){alert(e.message)}}
