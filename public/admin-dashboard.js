...
function logout(message=''){
  ...
  auth='';loginAt=0;location.replace('/admin'+(message?'?expired=1':''));
}
async function api(url,opt={}){
  if(Date.now()-loginAt>=SESSION_MS) logout(true);
  opt.headers={...(opt.headers||{}),Authorization:auth}; if(!(opt.body instanceof FormData)) opt.headers['Content-Type']='application/json';
...
const DEFAULT_NAV_ORDER=['dashboard','orders','products','keys','durations','settings','telegram','apk'];
...
function openPage(page){
  ...
  const titles={dashboard:'Dashboard',orders:'Orders',products:'Products',keys:'Product Keys',durations:'Product Duration',settings:'Settings',telegram:'Telegram Bot',apk:'APK Manager'};
...
window.addEventListener('hashchange',()=>{if(location.hash==='#apk')openPage('apk');});
setupNavDnD();
if(location.hash==='#apk') openPage('apk');
...
async function loadAdmin(){
  ...
  await loadApkManager();
}
async function loadApkManager(){
  const box=$('apkManagerList'); if(!box)return;
  try{const out=await api('/api/admin/apks'); const products=out?.products||[];
    box.innerHTML=products.length?products.map(p=>{const a=p.apk;return `<article class="order-card"><div class="order-top"><div><h3>${esc(p.name)}</h3><div class="order-meta">${a&&a.exists?'Current APK: '+esc(a.originalName)+' • '+formatFileSize(a.size):'No APK attached'}</div></div><span class="video-chip ${a&&a.exists?'success':''}">${a&&a.exists?'READY':'NOT SET'}</span></div><div class="admin-controls"><input id="apk-${escAttr(p.id)}" type="file" accept=".apk,application/vnd.android.package-archive"><button onclick="uploadProductApk('${escAttr(p.id)}')">${a&&a.exists?'Replace APK':'Upload APK'}</button>${a&&a.exists?`<a class="manage-btn" href="/admin/apk/download/${encodeURIComponent(p.id)}" target="_blank">Download</a><button class="danger-btn" onclick="removeProductApk('${escAttr(p.id)}')">Remove APK</button>`:''}</div><p class="video-muted">Maximum 250 MB. Only .apk files are accepted.</p></article>`}).join(''):`<p class="video-muted">No products yet.</p>`;
  }catch(e){box.innerHTML=`<p class="video-muted">${esc(e.message||'Unable to load APK Manager.')}</p>`}
}
function formatFileSize(n){n=Number(n||0);if(!n)return '—';const u=['B','KB','MB','GB'];let i=0;while(n>=1024&&i<3){n/=1024;i++}return `${n.toFixed(i?1:0)} ${u[i]}`}
async function uploadProductApk(id){const input=$('apk-'+id);if(!input?.files?.[0])return alert('Select an APK file first.');const file=input.files[0];if(!file.name.toLowerCase().endsWith('.apk'))return alert('Only .apk files are allowed.');if(file.size>250*1024*1024)return alert('APK is too large. Maximum size is 250 MB.');const fd=new FormData();fd.append('apk',file);try{const out=await api('/api/admin/products/'+encodeURIComponent(id)+'/apk',{method:'POST',body:fd});alert(out.message||'APK uploaded.');await loadAdmin()}catch(e){alert(e.message)}}
async function removeProductApk(id){if(!confirm('Remove this product APK?'))return;try{const out=await api('/api/admin/products/'+encodeURIComponent(id)+'/apk',{method:'DELETE'});alert(out.message||'APK removed.');await loadAdmin()}catch(e){alert(e.message)}}
