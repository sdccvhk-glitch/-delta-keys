...
if(new URLSearchParams(location.search).get('view')==='apk') openPage('apk');
...
async function loadAdmin(){
  ...
  renderStock(products,pool);renderProducts(products);renderPlans(products);renderOrders(orders);renderApkManager(products);refreshUploadPlans();
}
...
function renderApkManager(products){
  const box=$('apkManagerList'); if(!box)return;
  if(!products.length){box.innerHTML='<p class="video-muted">No products yet.</p>';return;}
  const size=n=>{n=Number(n||0);if(!n)return '—';let u=['B','KB','MB','GB'],i=0;while(n>=1024&&i<3){n/=1024;i++}return `${n.toFixed(i?1:0)} ${u[i]}`};
  box.innerHTML=products.map(p=>{const a=p.apk;return `<article class="order-card"><div class="order-top"><div><h3>${esc(p.name)}</h3><div class="order-meta">Product ID: ${esc(p.id)}<br>${a?'Current APK: '+esc(a.originalName||'APK')+' • '+size(a.size):'No APK attached'}</div></div><span class="video-chip ${a&&a.exists?'success':''}">${a&&a.exists?'APK ATTACHED':'NO APK'}</span></div><div class="admin-controls"><input id="apk-${escAttr(p.id)}" type="file" accept=".apk,application/vnd.android.package-archive"><button onclick="uploadProductApk('${escAttr(p.id)}')">${a&&a.exists?'Replace APK':'Upload APK'}</button>${a&&a.exists?`<button class="danger-btn" onclick="removeProductApk('${escAttr(p.id)}')">Remove APK</button>`:''}</div><div class="order-meta">Maximum 250 MB • Only .apk files</div></article>`}).join('');
}
async function uploadProductApk(id){
  const input=$('apk-'+id); if(!input||!input.files[0]){alert('Select an APK file first.');return;}
  try{
    const r=await fetch('/api/admin/products/'+encodeURIComponent(id)+'/apk',{method:'POST',headers:{Authorization:auth},body:(()=>{const fd=new FormData();fd.append('apk',input.files[0]);return fd})()});
    let out={};try{out=await r.json()}catch{}
    if(r.status===401){logout(true);return;}
    if(!r.ok)throw Error(out.error||'APK upload failed.');
    alert('APK uploaded successfully.'); await loadAdmin(); openPage('apk');
  }catch(e){alert(e.message)}
}
async function removeProductApk(id){
  if(!confirm('Remove this product APK?'))return;
  try{await api('/api/admin/products/'+encodeURIComponent(id)+'/apk',{method:'DELETE'});alert('APK removed.');await loadAdmin();openPage('apk')}catch(e){alert(e.message)}
}
